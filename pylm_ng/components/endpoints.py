...
class PubEndPoint(object):
    """
    Pub endpoint to test sub connections
    """
    def __init__(self, bind_address="inproc://PubEndPoint", logger=None):
        self.socket = zmq_context.socket(zmq.PUB)
        self.socket.bind(bind_address)
        self.bind_address = bind_address
        self.logger = logger

    def start(self, function='none', payload=b'pub', nmessages=10):
        """
        Start the endpoint, Sending some messages.
        :param function: Function to call within the RPC mechanism.
        :param payload: Payload that is sent with the message
        :param nmessages: Number of test messages to be sent
        :return:
        """
        for i in range(nmessages):
            message = PalmMessage()
...
class SubEndPoint(object):
    """
    Sub endpoint to test push connections
    """
    def __init__(self, bind_address="inproc://SubEndPoint", logger=None):
    ...
    def start(self, nmessages=10):
        """
        Start the endpoint, receiving some messages
        :param nmessages: Number of messages before shutting down.
        :return:
        """

        for i in range(nmessages):
            message_data = self.socket.recv()
            self.logger.info('Sub endpoint got meesage #{}'.format(i))